...
import ExpressionE (ConstrainedType, Id, PackageName, showPackage)
import Lexer (Token)
...
import Reader (ReaderOptions (..), readFromStrings, readPackageOrFail)
...
usage full = do
  ...
  if full
    then do
      ...
      putStr ("\nMejoras\n")
      putStr
        ( "--manija <n> : Cantidad de rondas de optimización "
  ...
  putStr
    ( "  qr Fulano --c -o f.c && gcc -o f f.c -Wall && ./f  "
        ++ "# ejecuta en C\n"
    )
  exitFailure
  where
...
parseOptions = do
  ...
  where
    ...
    rec ("--py" : args) options =
      rec
        args
        ( Map.insert OptQ_OutputLanguage "Py" $
            Map.insert OptQ_Compile "" options
        )
    rec ("--jvm" : args) options =
      rec
        args
        ( Map.insert OptQ_OutputLanguage "Jvm" $
            Map.insert OptQ_Compile "" options
        )
    rec ("--c" : args) options =
      rec
        args
        ( Map.insert OptQ_OutputLanguage "C" $
            Map.insert OptQ_Compile "" options
        )
    -- Name of main
...
runScript options packageStr = do
  ...
  if Map.member OptQ_OnlyType options
    then case inferPackages options packages of
