...
import CPS (CpsOptions (..), cps, cpsOrFail)
...
import ExpressionE
import ExpressionK
import Infer (InferOptions (..), infer, inferOrFail)
import Invariants (exprKInvariant)
import Lexer (Token)
import Optim (OptimOptions (..), contractUntilNoChanges, optimizeExpr)
import Parser
  ( ParserOptions (..),
...
import System.IO (IOMode (..), hClose, hPutStr, openBinaryFile)
...
evalString options mainName string = do
  let exprK =
        cpsOrFail (recursionOnData options)
          . precompileOrFail
          . fst
          . inferOrFail (valueRestriction options)
          . parseFromStringOrFail
            (parserOptionsFor options)
            globalPackage
            mainName
  ...
  where
    ...
    checkExprKInvariant description expr = do
      if Map.member OptQ_CheckInternalInvariants options
        then case exprKInvariant expr of
          Left msg ->
            Left
...
generateCode options pragmas "Py" expr = do
  let pySource = compileToPy pyOptions pragmas expr
   in case Map.findWithDefault "-" OptQ_OutputFile options of
